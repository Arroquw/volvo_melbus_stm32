...
#define MELBUS_CLOCKBIT (byte)PA2 //Pin D2  - CLK
#define MELBUS_DATA (byte)PA3     //Pin D3  - Data
#define MELBUS_BUSY (byte)PA4     //Pin D4  - Busy
#define CLOCKBIT_MASK 0x4
#define DATA_MASK 0x8
#define BUSY_MASK 0x10

const byte mutePin = PA5;
const byte playPin = PA6;
const byte prevPin = PA7;
const byte nextPin = PB0;
const byte upPin = PB1;    //volume up (not used on PCB)
const byte downPin = PB10;  //volume down (not used on PCB)
...
const float T_LOSS = 0.5; //if circuit is protected with a transistor and diode, put total voltage drop here.

byte track = 0x09; //Display show HEX value, not DEC. (A-F not "allowed")
byte cd = 0x09; //1-10 is allowed (in HEX. 0A-0F and 1A-1F is not allowed)
...
void stmSetPinMode(GPIO_TypeDef *gpio, uint32_t pin, uint8_t mode) {
...
typedef enum {
  ...
  GPIO_PUPDR_PULLDOWN = 0b10,
}GPIO_PUPDR;

void stmSetPuPd(GPIO_TypeDef *gpio, uint32_t pin, uint8_t mode) {
...
void melbusInitReq() {
  ...
  GPIOA->BSRR = BUSY_MASK;
  
  stmSetPinMode(GPIOA, MELBUS_BUSY, GPIO_MODER_INPUT);
  stmSetPuPd(GPIOA, MELBUS_BUSY, GPIO_PUPDR_PULLUP);
  //pinMode(MELBUS_BUSY, INPUT_PULLUP);
...
void SendByteToMelbus() {
  ...
  stmSetPinMode(GPIOA, MELBUS_DATA, GPIO_MODER_OUTPUT);

  //For each bit in the byte
  for (char i = 7; i >= 0; i--)
  {
    while ((GPIOA->IDR & BUSY_MASK)) {} //wait for low clock
    //If bit [i] is "1" - make datapin high
    if (byteToSend & (1 << i)) {
      GPIOA->BSRR = DATA_MASK;
    }
    //if bit [i] is "0" - make datapin low
    else {
      GPIOA->BSRR = DATA_MASK << 16;
    }
    while (!(GPIOA->IDR & BUSY_MASK)) {}  //wait for high clock
  }
  ...
  GPIOA->BSRR = DATA_MASK;
  //pinMode(MELBUS_DATA, INPUT_PULLUP);
  stmSetPinMode(GPIOA, MELBUS_DATA, GPIO_MODER_INPUT);
  stmSetPuPd(GPIOA, MELBUS_DATA, GPIO_PUPDR_PULLUP);
...
void SendByteToMelbus2() {
  ...
  {
    delayMicroseconds(7);
    GPIOA->BSRR = CLOCKBIT_MASK << 16;  //clock -> low
    //If bit [i] is "1" - make datapin high
    if (byteToSend & (1 << i)) {
      //digitalWrite(MELBUS_DATA, HIGH);
      GPIOA->BSRR = DATA_MASK;
    }
    //if bit [i] is "0" - make datapin low
    else {
    ...
    delayMicroseconds(5);
    GPIOA->BSRR = CLOCKBIT_MASK;   //clock -> high
...
void SendText(byte rowNum) {
  ...
  //pinMode(MELBUS_DATA, OUTPUT); //To slow, use DDRD instead:
  //digitalWrite(MELBUS_DATA, HIGH);      //set DATA input_pullup => HIGH when output (idle, HIGH)
  GPIOA->BSRR = DATA_MASK;
  //pinMode(MELBUS_DATA, OUTPUT);     //set DATA as output
  stmSetPinMode(GPIOA, MELBUS_DATA, GPIO_MODER_OUTPUT);
  //digitalWrite(MELBUS_CLOCKBIT, HIGH);  //set CLK input_pullup => HIGH when output (idle, HIGH)
  GPIOA->BSRR = CLOCKBIT_MASK;
  //pinMode(MELBUS_CLOCKBIT, OUTPUT); //set CLK as output
  stmSetPinMode(GPIOA, MELBUS_CLOCKBIT, GPIO_MODER_OUTPUT);
  ...
  stmSetPinMode(GPIOA, MELBUS_CLOCKBIT, GPIO_MODER_INPUT);
  stmSetPuPd(GPIOA, MELBUS_CLOCKBIT, GPIO_PUPDR_PULLUP);
  //pinMode(MELBUS_CLOCKBIT, INPUT_PULLUP);//back to input (PULLUP since we clocked in the last bit with clk = high, INPUT_PULLUP)
  //Reset datapin to high and return it to an input
  //pinMode(MELBUS_DATA, INPUT_PULLUP);
  //PORTD |= 1 << MELBUS_DATA;  //this may or may not change the state, depending on the last transmitted bit
  //digitalWrite(MELBUS_DATA, HIGH);
  GPIOA->BSRR = DATA_MASK;
  //pinMode(MELBUS_DATA, INPUT_PULLUP);//release the data line
  stmSetPinMode(GPIOA, MELBUS_DATA, GPIO_MODER_INPUT);
  stmSetPuPd(GPIOA, MELBUS_DATA, GPIO_PUPDR_PULLUP);
  //Clear INT flag
  //EIFR |= 1 << INT_NUM;
...
void reqMaster() {
  //pinMode(MELBUS_DATA, OUTPUT); //output
  stmSetPinMode(GPIOA, MELBUS_DATA, GPIO_MODER_OUTPUT);
  //digitalWrite(MELBUS_DATA, LOW);//low
  GPIOA->BSRR = DATA_MASK << 16;
  delayMicroseconds(700);
  delayMicroseconds(800);
  delayMicroseconds(800);
  //digitalWrite(MELBUS_DATA, HIGH);//high
  GPIOA->BSRR = DATA_MASK;
  //pinMode(MELBUS_DATA, INPUT_PULLUP); //back to input_pullup
  stmSetPinMode(GPIOA, MELBUS_DATA, GPIO_MODER_INPUT);
  stmSetPuPd(GPIOA, MELBUS_DATA, GPIO_PUPDR_PULLUP);
}
