...
*/
#define MELBUS_CLOCKBIT PA2 //Pin D2  - CLK
#define MELBUS_DATA PA3     //Pin D3  - Data
#define MELBUS_BUSY PA4     //Pin D4  - Busy
#define MELBUS_CLOCKBIT_PIN GPIO_PIN_2
...
byte md = 0x01; //1-4 is allowed (in HEX.)
...
#define MD_RESPONSE_ID 0xDE  //ID while responding to init requests (which will use base_id)
#define MD_MASTER_ID 0xDF    //ID while requesting/beeing master
#define MD_BASE_ID 0xD8      //ID when getting commands from HU
#define MD_ALT_ID 0xD9       //Alternative ID when getting commands from HU
...
#define CDC_CIR {3, MD_BASE_ID, 0x1E, 0xEF}             //Cartridge info request. Respond with 6 bytes
#define CDC_TIR {5, MD_ALT_ID, 0x1B, 0xE0, 0x01, 0x08}  //track info req. resp 9 bytes
#define CDC_NXT {5, MD_BASE_ID, 0x1B, 0x2D, 0x40, 0x01} //next track.
#define CDC_PRV {5, MD_BASE_ID, 0x1B, 0x2D, 0x00, 0x01} //prev track
#define CDC_CHG {3, MD_BASE_ID, 0x1A, 0x50}             //change cd
#define CDC_PUP {3, MD_BASE_ID, 0x19, 0x2F}             //power up. resp ack (0x00).
#define CDC_PDN {3, MD_BASE_ID, 0x19, 0x22}             //power down. ack (0x00)
#define CDC_FFW {3, MD_BASE_ID, 0x19, 0x29}             //FFW. ack
#define CDC_FRW {3, MD_BASE_ID, 0x19, 0x26}             //FRW. ack
#define CDC_SCN {3, MD_BASE_ID, 0x19, 0x2E}             //scan mode. ack
#define CDC_RND {3, MD_BASE_ID, 0x19, 0x52}             //random mode. ack
#define CDC_NU {3, MD_BASE_ID, 0x1A, 0x50}              //not used
...
byte trackInfo[] = {0x00, 0x02, 0x00, cd, 0x80, track, 0x0C, 0xCC, 0xCC}; //9 bytes
byte startByte = 0x08; //on powerup - change trackInfo[1] & [8] to this
byte stopByte = 0x02; //same on powerdown
byte cartridgeInfo[] = {0x80, 0x00, 0x0F, 0x04, 0x00, 0x0F};
...
detachInterrupt(digitalPinToInterrupt(MELBUS_CLOCKBIT));
...
void SendByteToMelbus() {
  ...
  HAL_GPIO_WritePin(GPIOA, MELBUS_DATA_PIN, GPIO_PIN_RESET); // Set DATA input_pullup => HIGH when output (idle)
  GPIO_InitTypeDef GPIO_InitStruct = {0};


  GPIO_InitStruct.Pin = MELBUS_DATA_PIN;
  ...
  for (char i = 7; i >= 0; i--) {
    while (HAL_GPIO_ReadPin(GPIOA, MELBUS_CLOCKBIT_PIN) == GPIO_PIN_SET) {} // Wait for low clock

    // If bit [i] is "1" - make data pin high
    if (byteToSend & (1 << i)) {
      HAL_GPIO_WritePin(GPIOA, MELBUS_DATA_PIN, GPIO_PIN_SET);
    }
    // If bit [i] is "0" - make data pin low
    else {
      HAL_GPIO_WritePin(GPIOA, MELBUS_DATA_PIN, GPIO_PIN_RESET);
    }

    while (HAL_GPIO_ReadPin(GPIOA, MELBUS_CLOCKBIT_PIN) == GPIO_PIN_RESET) {} // Wait for high clock
  }

  // Let the value be read by the HU
  delayMicroseconds(20);

  // Reset data pin to high and return it to an input
  HAL_GPIO_WritePin(GPIOA, MELBUS_DATA_PIN, GPIO_PIN_SET);  // This may or may not change the state, depending on the last transmitted bit
  GPIO_InitStruct.Mode = GPIO_MODE_INPUT; // Input mode with pull-up
  HAL_GPIO_Init(GPIOA, &GPIO_InitStruct);
  attachInterrupt(digitalPinToInterrupt(MELBUS_CLOCKBIT), MELBUS_CLOCK_INTERRUPT, RISING);
}
...
void SendByteToMelbus2() {
  ...
  // int takes more clock cycles to update on an 8-bit CPU.
  for (char i = 7; i >= 0; i--) {
    delayMicroseconds(7);
    HAL_GPIO_WritePin(GPIOA, MELBUS_CLOCKBIT_PIN, GPIO_PIN_RESET); // Clock -> Low (Replace X with the actual clock pin)

    // If bit [i] is "1" - make data pin high
    if (byteToSend & (1 << i)) {
      HAL_GPIO_WritePin(GPIOA, MELBUS_DATA_PIN, GPIO_PIN_SET); // Data pin high
    } else {
      HAL_GPIO_WritePin(GPIOA, MELBUS_DATA_PIN, GPIO_PIN_RESET); // Data pin low
    }

    // Wait for output to settle
    delayMicroseconds(5);
    HAL_GPIO_WritePin(GPIOA, MELBUS_CLOCKBIT_PIN, GPIO_PIN_SET); // Clock -> High (Replace X with the actual clock pin)
  }
  delayMicroseconds(20);
}
...
// Convert datapin and clockpin to output
  // Set DATA and CLK pins as outputs with push-pull mode
  GPIO_InitTypeDef GPIO_InitStruct = {0};
  __HAL_RCC_GPIOA_CLK_ENABLE(); // Enable the clock for GPIOA (adjust the port as needed)
  HAL_GPIO_WritePin(GPIOA, MELBUS_DATA_PIN, GPIO_PIN_SET);
  HAL_GPIO_WritePin(GPIOA, MELBUS_CLOCKBIT_PIN, GPIO_PIN_SET);
  GPIO_InitStruct.Pin = MELBUS_DATA | MELBUS_CLOCKBIT;
  GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP; // Output push-pull mode
  GPIO_InitStruct.Pull = GPIO_NOPULL; // No pull-up or pull-down
  HAL_GPIO_Init(GPIOA, &GPIO_InitStruct);
  ...
  // Set CLK pin back to input (PULLUP since we clocked in the last bit with CLK = high)
  GPIO_InitStruct.Pin = MELBUS_CLOCKBIT;
  GPIO_InitStruct.Mode = GPIO_MODE_INPUT; // Input mode with pull-up
  GPIO_InitStruct.Pull = GPIO_PULLUP;
  HAL_GPIO_Init(GPIOA, &GPIO_InitStruct);

  // Reset DATA pin to high and return it to an input
  // Set DATA pin as input with pull-up
  HAL_GPIO_WritePin(GPIOA, MELBUS_DATA_PIN, GPIO_PIN_SET);
  GPIO_InitStruct.Pin = MELBUS_DATA;
  HAL_GPIO_Init(GPIOA, &GPIO_InitStruct);

  // Enable interrupt on INT_NUM (replace INT_NUM with the actual interrupt number)
  attachInterrupt(digitalPinToInterrupt(MELBUS_CLOCKBIT), MELBUS_CLOCK_INTERRUPT, RISING); // Enable the interrupt
}

void reqMaster() {
  // Set PA3 as an output
  GPIO_InitTypeDef GPIO_InitStruct = {0};
  __HAL_RCC_GPIOA_CLK_ENABLE(); // Enable the clock for GPIOA
  ...
  // Set PA3 back to input with pull-up
