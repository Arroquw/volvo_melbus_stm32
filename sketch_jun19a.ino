...
const byte mutePin = 12;
const byte playPin = 7;
const byte prevPin = 6;
const byte nextPin = 5;
const byte upPin = 13;    //volume up (not used on PCB)
const byte downPin = 22;  //volume down (not used on PCB)
...
byte cd = 0x04; //1-4 is allowed (in HEX. 0A-0F and 1A-1F is not allowed)
...
#define CDC_RESPONSE_ID 0xDE  //ID while responding to init requests (which will use base_id)
#define CDC_MASTER_ID 0xDF    //ID while requesting/beeing master
#define CDC_BASE_ID 0xD8      //ID when getting commands from HU
#define CDC_ALT_ID 0xD9       //Alternative ID when getting commands from HU
...
#define CDC_CIR {3, CDC_BASE_ID, 0x1E, 0xF9}             //Cartridge info request. Respond with 6 bytes
...
byte trackInfo[] = {0xB0, 0x08, 0x00, cd, 0x40, track, 0x0c, 0xcc, 0xcc}; //9 bytes
byte startByte = 0x00; //on powerup - change trackInfo[1] & [8] to this
byte stopByte = 0x08; //same on powerdown
byte cartridgeInfo[] = {0x00, 0x03, 0xFF, 0x4A, 0x03, 0xFF};
            ...
            switch (cmd) {
              ...
              case 5:
                ...
                while (!(PIND & (1 << MELBUS_BUSY))) {
                  if (byteIsRead) {
                    byteIsRead = false;
                    if (melbus_ReceivedByte == CDC_MASTER_ID) {
                      byteToSend = CDC_MASTER_ID;
                      SendByteToMelbus();
                      SendText(textRow);
                      //Serial.println("MRB2");
                      break;
